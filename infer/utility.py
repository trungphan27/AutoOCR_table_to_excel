...
def init_args():
    ...
    parser.add_argument(
        ...
        help=("Comma-separated ONNX Runtime execution providers. "
              "Use 'auto' to prefer CUDA when --use_gpu=True."))
    parser.add_argument("--onnx_intra_op_threads", type=int, default=0)
    parser.add_argument("--onnx_inter_op_threads", type=int, default=0)
    parser.add_argument("--onnx_graph_optimization", type=str, default="all")
    parser.add_argument("--onnx_execution_mode", type=str, default="sequential")
    parser.add_argument("--onnx_enable_mem_pattern", type=str2bool, default=True)
    parser.add_argument(
        "--onnx_enable_cpu_mem_arena", type=str2bool, default=True)
    parser.add_argument("--onnx_log_severity", type=int, default=3)
    parser.add_argument("--onnx_cuda_device_id", type=int, default=0)
    parser.add_argument("--onnx_cuda_mem_limit_mb", type=int, default=0)
    parser.add_argument(
        "--onnx_cuda_arena_extend_strategy",
        type=str,
        default="kNextPowerOfTwo")
    parser.add_argument(
        "--onnx_cuda_cudnn_conv_algo_search",
        type=str,
        default="EXHAUSTIVE")
    parser.add_argument("--onnx_cuda_use_tf32", type=str2bool, default=True)
    return parser
...
def create_predictor(args, mode, logger):
    ...
    if args.use_onnx:
        ...
        provider_override = getattr(
            args, "onnx_{}_providers".format(mode), None
        )
        sess = create_onnx_session(
            model_dir, args, providers_override=provider_override
        )
