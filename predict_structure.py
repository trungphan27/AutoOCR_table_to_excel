...
import utility
from infer.onnx_runtime import prepare_onnx_input, prepare_onnx_outputs
...
class TableStructurer(object):
    ...
    def __call__(self, img):
        ...
        if self.use_onnx:
            input_dict = {}
            input_dict[self.input_tensor.name] = prepare_onnx_input(
                img, self.input_tensor)
            outputs = self.predictor.run(self.output_tensors, input_dict)
            outputs = prepare_onnx_outputs(outputs)
        else:
        ...
        if len(outputs) < 2:
            raise RuntimeError(
                "SLANet must expose cell-location and structure-probability outputs."
            )
        location_outputs = [
        ...
        if not location_outputs or not structure_outputs:
            shapes = [tuple(output.shape) for output in outputs]
            raise RuntimeError(
                "Cannot identify SLANet outputs from shapes: {}. Expected one "
                "output ending in 4 cell coordinates and one token-probability "
                "output.".format(shapes)
            )

        preds = {
            'structure_probs': structure_outputs[0],
            'loc_preds': location_outputs[0],
        }
